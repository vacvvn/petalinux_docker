...
- для удобства сборки компонентов в директории **хоста** `/absolute/path/to/host/folder` появится шаблон скрипта `component_update.sh`
	- этот скрипт следует отредактировать и запускать **на хосте** для сборки компонентов. Скрипт запустит контейнер и передаст в него команды `petalinux-build -c .....`.
...
При сборке образа в него распаковывается архив зависимостей `sstate`. После запуска контейнера этот архив используется для сборки проектов. Для его использования при конфигурации проекта в разделе `Yocto Setting->Local sstate feeds settings` нужно прописать путь до директории `aarch64` распакованного архива.
...
> [!NOTE] Внимание
> опыт показал, если на хосте открыта директория проекта, которая смонтирована в контейнер, то сборка и конфигурация завершается ошибкой.
